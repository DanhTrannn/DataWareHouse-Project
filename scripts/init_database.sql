...
-- Schema cho dữ liệu đã làm sạch và lưu lịch sử (chuẩn hóa 3NF)
CREATE SCHEMA core;
GO

-- Schema cho Data Mart (star schema)
CREATE SCHEMA datamart;
GO

SELECT name FROM sys.schemas WHERE name IN ('staging', 'core', 'datamart');
...
CREATE TABLE staging.stg_products (
    ...
    product_name_length         INT            NULL,  -- typo gốc trong dataset Olist
    product_description_length  INT            NULL,  -- typo gốc trong dataset Olist
...
-- ==========================================
-- BƯỚC 1.5: Tạo Core Tables (Cleansed Data)
-- ==========================================

-- 1.5.1 core.customers
...
CREATE TABLE core.customers (
    ...
    customer_unique_id       VARCHAR(50)   NOT NULL,
    customer_zip_code_prefix VARCHAR(10)   NOT NULL, -- Thay NULL = '00000'
    customer_city            NVARCHAR(100) NOT NULL, -- Thay NULL = 'Unknown'
    customer_state           VARCHAR(5)    NOT NULL  -- Thay NULL = 'NA'
);
GO

-- 1.5.2 core.geolocation
IF OBJECT_ID('core.geolocation', 'U') IS NOT NULL
    DROP TABLE core.geolocation;
GO
...
-- 1.5.3 core.order_reviews
IF OBJECT_ID('core.order_reviews', 'U') IS NOT NULL
...
CREATE TABLE core.order_reviews (
    review_id                VARCHAR(50)   NOT NULL,
    order_id                 VARCHAR(50)   NOT NULL,
    review_score             INT           NOT NULL, -- Thay NULL = 0
    review_comment_title     NVARCHAR(200) NOT NULL, -- Thay NULL = 'No Title'
    review_comment_message   NVARCHAR(MAX) NOT NULL, -- Thay NULL = 'No Comment'
...
-- 1.5.4 core.sellers
IF OBJECT_ID('core.sellers', 'U') IS NOT NULL
    DROP TABLE core.sellers;
GO
...
CREATE TABLE core.orders (
    ...
    order_approved_at              DATETIME     NULL, -- Giữ NULL vì có thể đơn chưa duyệt
    order_delivered_carrier_date   DATETIME     NULL,
    order_delivered_customer_date  DATETIME     NULL,
    order_estimated_delivery_date  DATETIME     NULL
);
...
-- 1.5.6 core.order_payments
IF OBJECT_ID('core.order_payments', 'U') IS NOT NULL
...
CREATE TABLE core.order_payments (
    ...
    payment_sequential    INT           NOT NULL,
    payment_type          VARCHAR(30)   NOT NULL,
    payment_installments  INT           NOT NULL, -- Thay NULL = 1
    payment_value         DECIMAL(10,2) NOT NULL
);
GO
...
CREATE TABLE core.products (
    ...
    product_category_name       NVARCHAR(100)  NOT NULL, -- Thay NULL = 'Unknown'
    product_name_length         INT            NOT NULL, -- Thay NULL = 0
    product_description_length  INT            NOT NULL, -- Thay NULL = 0
    product_photos_qty          INT            NOT NULL, -- Thay NULL = 0
    product_weight_g            DECIMAL(10,2)  NOT NULL, -- Thay NULL = 0
...
-- 1.5.8 core.category_translation
IF OBJECT_ID('core.category_translation', 'U') IS NOT NULL
    DROP TABLE core.category_translation;
GO
...
-- 1.5.9 core.order_items
IF OBJECT_ID('core.order_items', 'U') IS NOT NULL
...
-- 2.1. datamart.dim_date
IF OBJECT_ID('datamart.dim_date', 'U') IS NOT NULL
...
-- 2.2. datamart.dim_geolocation
IF OBJECT_ID('datamart.dim_geolocation', 'U') IS NOT NULL
    DROP TABLE datamart.dim_geolocation;
GO

CREATE TABLE datamart.dim_geolocation (
...
CREATE UNIQUE INDEX UX_dim_geolocation_zip ON datamart.dim_geolocation(zip_code_prefix);
GO

-- 2.3. datamart.dim_customer (SCD Type 2)
IF OBJECT_ID('datamart.dim_customer', 'U') IS NOT NULL
    DROP TABLE datamart.dim_customer;
GO
...
CREATE TABLE datamart.dim_customer (
    ...
    CONSTRAINT FK_dim_customer_geo FOREIGN KEY (geo_key)
        REFERENCES datamart.dim_geolocation(geo_key)
);
GO

CREATE INDEX IX_dim_customer_unique ON datamart.dim_customer(customer_unique_id, is_current);
GO

-- 2.4. datamart.dim_order_status
IF OBJECT_ID('datamart.dim_order_status', 'U') IS NOT NULL
    DROP TABLE datamart.dim_order_status;
GO

CREATE TABLE datamart.dim_order_status (
...
-- 3.1. datamart.fact_customer_orders (monthly)
IF OBJECT_ID('datamart.fact_customer_orders', 'U') IS NOT NULL
    DROP TABLE datamart.fact_customer_orders;
GO

CREATE TABLE datamart.fact_customer_orders (
    ...
    CONSTRAINT FK_fco_customer FOREIGN KEY (customer_key)
        REFERENCES datamart.dim_customer(customer_key),
    CONSTRAINT FK_fco_status FOREIGN KEY (order_status)
        REFERENCES datamart.dim_order_status(order_status),
...
SELECT TABLE_SCHEMA, TABLE_NAME
FROM INFORMATION_SCHEMA.TABLES
WHERE TABLE_SCHEMA IN ('staging', 'datamart')
ORDER BY TABLE_SCHEMA, TABLE_NAME;


SELECT 'dim_date'         AS tbl, COUNT(*) AS cnt FROM datamart.dim_date
UNION ALL SELECT 'dim_order_status',  COUNT(*) FROM datamart.dim_order_status
UNION ALL SELECT 'dim_geolocation',   COUNT(*) FROM datamart.dim_geolocation
UNION ALL SELECT 'dim_customer',      COUNT(*) FROM datamart.dim_customer;

-- Verify SCD Type 2: lần đầu tất cả is_current = 1
SELECT is_current, COUNT(*) FROM datamart.dim_customer GROUP BY is_current;
-- Expected: is_current=1 → ~99,441



-- TV3
--  datamart.dim_seller (SCD Type 2)
IF OBJECT_ID('datamart.dim_seller', 'U') IS NOT NULL
    DROP TABLE datamart.dim_seller;
GO

CREATE TABLE datamart.dim_seller (
...
CREATE INDEX IX_dim_seller_id ON datamart.dim_seller(seller_id, is_current);
GO

-- datamart.fact_order_lifecycle (Accumulating Snapshot)
IF OBJECT_ID('datamart.fact_order_lifecycle', 'U') IS NOT NULL
    DROP TABLE datamart.fact_order_lifecycle;
GO

CREATE TABLE datamart.fact_order_lifecycle (
    ...
    CONSTRAINT FK_fl_seller FOREIGN KEY (seller_key)
        REFERENCES datamart.dim_seller(seller_key)
);
GO

CREATE UNIQUE INDEX UX_fact_lifecycle_order ON datamart.fact_order_lifecycle(order_id);
GO

-- datamart.fact_order_lifecycle (Accumulating Snapshot)
IF OBJECT_ID('datamart.fact_delivery', 'U') IS NOT NULL
    DROP TABLE datamart.fact_delivery;
GO

CREATE TABLE datamart.fact_delivery (
...
-- datamart.dim_payment_method
IF OBJECT_ID('datamart.dim_payment_method', 'U') IS NOT NULL
    DROP TABLE datamart.dim_payment_method;
GO

CREATE TABLE datamart.dim_payment_method (
    payment_type  VARCHAR(30)   NOT NULL PRIMARY KEY,
    description   NVARCHAR(100) NULL
);
GO

IF OBJECT_ID('datamart.fact_payment_trends', 'U') IS NOT NULL
    DROP TABLE datamart.fact_payment_trends;
GO

CREATE TABLE datamart.fact_payment_trends (
    payment_type        VARCHAR(30)    NOT NULL,
    date_key            INT            NOT NULL,
    ...
    CONSTRAINT FK_fpt_payment FOREIGN KEY (payment_type)
        REFERENCES datamart.dim_payment_method(payment_type),
    CONSTRAINT FK_fpt_date FOREIGN KEY (date_key)
        REFERENCES datamart.dim_date(date_key)
);
GO

SELECT COUNT(*) AS total_rows FROM datamart.dim_seller;
SELECT is_current, COUNT(*) AS cnt
FROM datamart.dim_seller
GROUP BY is_current;
...
SELECT COUNT(*) FROM datamart.fact_order_lifecycle;

SELECT order_status, COUNT(*) AS cnt
FROM datamart.fact_order_lifecycle
...
-- datamart.dim_product_category
IF OBJECT_ID('datamart.dim_product_category', 'U') IS NOT NULL
    DROP TABLE datamart.dim_product_category;
GO

CREATE TABLE datamart.dim_product_category (
...
CREATE UNIQUE INDEX UX_dim_prodcat_name
    ON datamart.dim_product_category(category_name_portuguese);
GO

--  datamart.dim_product
IF OBJECT_ID('datamart.dim_product', 'U') IS NOT NULL
    DROP TABLE datamart.dim_product;
GO
...
CREATE TABLE datamart.dim_product (
    ...
    CONSTRAINT FK_dim_product_category FOREIGN KEY (category_key)
        REFERENCES datamart.dim_product_category(category_key)
);
GO

CREATE UNIQUE INDEX UX_dim_product_id ON datamart.dim_product(product_id);
GO

-- datamart.fact_orders
IF OBJECT_ID('datamart.fact_orders', 'U') IS NOT NULL
...
CREATE TABLE datamart.fact_orders (
    ...
    CONSTRAINT FK_fo_product FOREIGN KEY (product_key)
        REFERENCES datamart.dim_product(product_key),
...
CREATE UNIQUE INDEX UX_fact_orders_bk ON datamart.fact_orders(order_id, order_item_id);
GO

-- Indexes cho FK joins (cải thiện performance truy vấn)
CREATE INDEX IX_fo_customer ON datamart.fact_orders(customer_key);
CREATE INDEX IX_fo_seller   ON datamart.fact_orders(seller_key);
CREATE INDEX IX_fo_product  ON datamart.fact_orders(product_key);
CREATE INDEX IX_fo_date     ON datamart.fact_orders(order_date_key);
GO

-- datamart.fact_sales (monthly)
IF OBJECT_ID('datamart.fact_sales', 'U') IS NOT NULL
    DROP TABLE datamart.fact_sales;
GO

CREATE TABLE datamart.fact_sales (
    seller_key       INT            NOT NULL,
